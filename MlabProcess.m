...
SubFolderPath='OriginalData';%Sets the file path
SubFolders=dir(SubFolderPath); %Retrieves all folder names
NumSubFolders=size(SubFolders); %Determines the number of .mat files to load

for j = 1:NumSubFolders(1)
    res=[];
    SubFolderName=SubFolders(j).name;
    
    if isempty(strfind(SubFolderName, 'Sub')) ~= true && isempty(strfind(SubFolderName, 'Sub006')) ~= false
        FolderName=strcat(SubFolderPath, '/', SubFolderName, '/*.mat');
        ...
        for i = 1:NumFiles(1) %Iterates through each the length of FileList
            ...
            filepath= strcat(SubFolderPath, '/', SubFolderName, '/', name); %Concatenates the file name to form the full file path
